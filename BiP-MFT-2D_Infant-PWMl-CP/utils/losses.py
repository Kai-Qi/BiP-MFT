...
class DiceLoss(nn.Module):
    ...
    def forward(self, predict, target):
        ...
        assert temp_inputs.shape == temp_target.shape, 'predict & target shape do not match'
        dice = BinaryDiceLoss(**self.kwargs)
        total_loss = 0
        predict = F.softmax(temp_inputs, dim=1)

        for i in range(temp_target.shape[1]):
            if i != self.ignore_index:
                dice_loss = dice(predict[:, i], temp_target[:, i])
                if self.weight is not None:
                    assert self.weight.shape[0] == temp_target.shape[1], \
                        'Expect weight shape [{}], get[{}]'.format(temp_target.shape[1], self.weight.shape[0])
                    dice_loss *= self.weights[i]
                total_loss += dice_loss

        return total_loss/temp_target.shape[1]