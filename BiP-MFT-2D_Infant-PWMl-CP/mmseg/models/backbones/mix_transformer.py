...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W, opposite_kv=None, opposite=None):
        B, N, C = x.shape
        ...
        if opposite_kv==None:
            ...
            x_after_cross = back_ori
            
            q2 = self.q(x_after_cross).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            if self.sr_ratio > 1:
                x_ = x_after_cross.permute(0, 2, 1).reshape(B, C, H, W)
                x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
                x_ = self.norm(x_)
                kv2 = self.kv(x_).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            else:
                kv2 = self.kv(x_after_cross).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            k2, v2 = kv2[0], kv2[1]
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W, opposite_kv, opposite):
        att, kv = self.attn(self.norm1(x), H, W, opposite_kv, opposite)
...
class mit_b5(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(mit_b5, self).__init__(
            ...
            attn_drop_rate=0.1, drop_rate=0.1, drop_path_rate=0.1)